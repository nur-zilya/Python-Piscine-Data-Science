...
if __name__ == '__main__':
    ...
    out = Research(sys.argv[1])
    print(out.file_reader())
    data = out.Calculations(out.file_reader())
    calc = data.counts()
    print(calc[0], calc[1])
    frac = data.fractions(calc[0], calc[1])
    print(frac[0], frac[1])

    child = out.Analytics(out.file_reader())
    predict = child.predict_random(3)
    print(predict)
    print(child.predict_last(predict))
